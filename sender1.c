...
extern void seqsender(int np, int dport, char * src, char * dest, char * iface){
    seqPacketSender(np, dport, src, dest, iface);
}
...
void seqPacketSender(int np, int dport, char * source, char * destination, char * iface) {

    FILE *fr;
    fr = fopen ("seq_num.txt", "r");
    fseek(fr, 0, SEEK_END);
    long fsize = ftell(fr);
    fseek(fr, 0, SEEK_SET);
    char line[1400];
